...
#if (CONFIG_SOC_SMP)
#include "smp.h"
__STATIC_FORCEINLINE void __tcm_start(void)
{
	typedef struct {
		uint32_t cpu_id;
		uint32_t const *src;
	...
	extern const __copy_table_t __tcm_copy_table_start__;
	extern const __copy_table_t __tcm_copy_table_end__;

	for (__copy_table_t const* pTable = &__tcm_copy_table_start__; pTable < &__tcm_copy_table_end__; ++pTable) {
		if(cpu_get_core_id() == pTable->cpu_id){
...
#define __TCM_LOADER_START         __tcm_start
#endif
