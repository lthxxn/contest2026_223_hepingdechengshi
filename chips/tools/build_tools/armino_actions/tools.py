...
def _armino_version_from_cmake():
    version_path = os.path.join(os.environ['ARMINO_TOOLS_PATH'], 'cmake/version.cmake')
...
def armino_version():
    ...
    try:
        version = subprocess.check_output([
            ...
            '--work-tree=%s' % os.environ['ARMINO_PATH'],
        ...
        version = _armino_version_from_cmake()

    return version


def run_tool(tool_name, args, cwd, env=dict()):
    print(f"run tool: {tool_name} {args}")
    def quote_arg(arg):
    ...
    try:
        # Note: we explicitly pass in os.environ here, as we may have set ARMINO_PATH there during startup
        print(f"Final Cmd: {args}")
...
def run_target(target_name, args, env=dict()):
    ...
    print("Run Tool:", generator_cmd[0], generator_cmd + [target_name], args.build_dir, env)
...
def ensure_build_directory(args, prog_name, always_run_cmake=False):
    ...
    print(f"ensure_build_directory {project_dir}")
    ...
    toolchain_dir = realpath(args.toolchain_dir)
...
def is_target_supported(project_path, supported_targets):
    ...
    """
    return get_sdkconfig_value(os.path.join(project_path, 'sdkconfig'), 'CONFIG_SOC_STR') in supported_targets
...
def _guess_or_check_armino_target(args, prog_name, cache):
    ...
    if not cache and not armino_target_from_env:
        ...
        if armino_target_from_cache and armino_target_from_cache != armino_target_from_env:
            raise FatalError("Target settings are not consistent: '{t_env}' in the environment, '{t_cache}' in CMakeCache.txt. "
                             ...
                             .format(t_env=armino_target_from_env, t_cache=armino_target_from_cache, prog=prog_name))

    elif armino_target_from_cache and armino_target_from_sdkconfig and armino_target_from_cache != armino_target_from_sdkconfig:
        ...
        raise FatalError("Project sdkconfig was generated for target '{t_conf}', but CMakeCache.txt contains '{t_cache}'. "
                         ...
                         .format(t_conf=armino_target_from_sdkconfig, t_cache=armino_target_from_cache, prog=prog_name))
